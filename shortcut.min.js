...
// 0.2.2 функци для работы в среде windows script host
(function(r,u,v){r.wsh={getLDAP:function(b,d,a,e,f){var l=[],p=[],c=0;if(!c){var n=new ActiveXObject("ADODB.Connection");var k=new ActiveXObject("NameTranslate");var m=new ActiveXObject("ADODB.Command")}c||(n.provider="ADsDSOObject",e&&(n.properties("User ID").value=e),f&&(n.properties("Password").value=f),n.open("Active Directory Provider"),m.activeConnection=n,m.properties("Searchscope").value=2,m.properties("Page Size").value=100);if(!c)if(a)var h=a;else try{var g=GetObject("LDAP://RootDSE");h=
g.get("DefaultNamingContext")}catch(t){c=1}c||(p=[b],d&&p.push(d));for(e=p.length-1;!c&&-1<e;e--){b=p[e];d=g=null;f="SELECT distinguishedName FROM 'LDAP://"+h+"'";if(!c&&b&&!d&&r.lib.validate(b,"guid")){a=(""+b).toUpperCase();try{k.init(3,""),k.set(7,a),d=k.get(1)}catch(t){c=2}}if(!c&&b&&!d)try{(d=b.get("distinguishedName"))&&(g=b)}catch(t){}if(!c&&b&&!d){a=f+" WHERE name = '"+b+"' OR sAMAccountName = '"+b+"' OR distinguishedName = '"+b+"'";try{m.commandText=a;var q=m.execute();q.recordCount&&(d=
q.fields("distinguishedName").value)}catch(t){}}if(!c&&!d){b?(a=""+b,a=r.lib.template(a,{protocol:"LDAP:",parent:h,select:f}),a.toUpperCase().indexOf("WHERE")?a.toUpperCase().indexOf("SELECT")&&(c=3):a=f+" "+a):a=f+" WHERE distinguishedName = '"+h+"'";try{m.commandText=a,q=m.execute(),q.recordCount||(c=5)}catch(t){c=4}}if(!c&&d&&!e)if(g)l.push(g);else{a=d;try{g=GetObject("LDAP://"+a),l.push(g)}catch(t){c=6}}if(!c&&!d)for(b=0,f=q.recordCount;b<f;q.moveNext(),b++)a=q.fields("distinguishedName").value,
b||(d=a),!e&&a&&(g=GetObject("LDAP://"+a),l.push(g));c||(h=d)}return l},iconv:function(b,d,a){var e=new ActiveXObject("ADODB.Stream");e.type=2;e.mode=3;e.open();e.charset=d;e.writeText(a);e.position=0;e.charset=b;a=e.readText();e.close();return a},setShortcut:function(b){var d,a,e,f={},l=!1,p=!1,c=0;var n=new ActiveXObject("WScript.Shell");var k=new ActiveXObject("Scripting.FileSystemObject");c||(b&&b.fullName?(b.targetPath&&(a=n.expandEnvironmentStrings(b.targetPath)),b.fullName&&(e=n.expandEnvironmentStrings(b.fullName))):
c=1);if(!c&&"targetPath"in b&&!a&&(l=!0,k.fileExists(e)))try{k.deleteFile(e,!0)}catch(g){c=3}if(!c&&!l)try{f=n.createShortcut(e)}catch(g){c=2}if(!c&&!l)for(d in b){var m=!0;a=b[d];switch(d.toLowerCase()){case "fullname":m=!1;break;case "targetpath":var h=n.expandEnvironmentStrings(a);h=k.getAbsolutePathName(h);f[d]==h&&(m=!1);h=k.getAbsolutePathName(a);!r.lib.compare(h,a,!0)&&r.lib.compare(h,a,!1)&&(a=h);break;case "iconlocation":a=~a.indexOf(",")?a.split(", ").join(","):a+",0"}if(m&&d in f&&f[d]!=
a)try{f[d]=a,p=!0}catch(g){c=4}}if(!c&&p)try{f.save()}catch(g){c=5}if(!c&&!l&&k.getFileName(e)!=k.getFileName(k.getAbsolutePathName(f.fullName)))try{k.moveFile(f.fullName,e)}catch(g){c=6}return c?!1:f},getFileText:function(b){var d="";var a=new ActiveXObject("ADODB.Stream");a.type=2;a.mode=3;a.open();try{a.loadFromFile(b),d=a.readText()}catch(e){}a.close();return d},setFileText:function(b,d,a){var e=0;var f=new ActiveXObject("ADODB.Stream");f.type=2;f.mode=3;f.open();if(a)try{f.loadFromFile(b),f.readText(),
f.writeText("\r\n")}catch(l){}try{f.writeText(d),f.saveToFile(b,2)}catch(l){e=1}f.close();return!e}}})(App.prototype,WSH);

// 0.2.0 создание, изменение и удаление ярлычка
var shortcut=new App({lnkExt:"lnk",urlExt:"url","escСhar":"^",extDelim:"."});
(function(l,f,t){f.lib.extend(f,{fun:{},init:function(){var d,e,a={},g=0;var h=new ActiveXObject("WScript.Shell");var m=new ActiveXObject("Scripting.FileSystemObject");if(!g){var k=l.arguments.length;for(e=0;e<k;e++){var b=l.arguments.item(e);var c="location";if(0!=e||c in a)if(c="name",1!=e||c in a)if(c="target",2!=e||c in a)if(c="argument",3!=e||c in a)if(c="directory",4!=e||c in a)if(c="description",5!=e||c in a)if(c="icon",6!=e||c in a)if(c="style",7!=e||c in a)if(c="hotkey",8!=e||c in a)break;
else a[c]=b;else b=Number(b),a[c]=b;else a[c]=b;else a[c]=b;else(d=b?h.specialFolders(b):null)&&(b=d),a[c]=b;else b=b.split("'").join('"'),a[c]=b;else(d=b?h.specialFolders(b):null)&&(b=d),a[c]=b;else a[c]=b;else(d=b?h.specialFolders(b):null)&&(b=d),a[c]=b}}g||"location"in a&&"name"in a||(g=1);!g&&"style"in a&&!f.lib.hasValue([1,3,7],a.style,!1)&&(g=2);if(!g){b={};var p=a.target&&~a.target.indexOf("/");k=!1;e=a.target?[p?f.val.urlExt:f.val.lnkExt]:[f.val.urlExt,f.val.lnkExt];for(var n=0,q=e.length;n<
q&&!k;n++){d=e[n];d=a.name+f.val.extDelim+d;d=m.buildPath(a.location,d);d=d.split(f.val.esc\u0421har).join("");var r=h.expandEnvironmentStrings(d);m.fileExists(r)&&(k=!0)}b.fullName=d;(d=a.target)&&!p&&(d=d.split(f.val.esc\u0421har).join(""),h=h.expandEnvironmentStrings(d),m.fileExists(h)||(d=null));"target"in a&&(b.targetPath=d);for(c in a)f.lib.hasValue(["argument","directory","icon"],c,!0)&&(d=a[c],d=d.split(f.val.esc\u0421har).join(""),a[c]=d);"argument"in a&&(b.arguments=a.argument);"directory"in
a&&(b.workingDirectory=a.directory);"description"in a&&(b.description=a.description);"icon"in a&&(b.iconLocation=a.icon);"style"in a&&(b.windowStyle=a.style);"hotkey"in a&&(b.hotkey=a.hotkey);(obj=f.wsh.setShortcut(b))||(g=3)}l.quit(g)}})})(WSH,shortcut);shortcut.init();
